<script>
    ...
    $('#deleteTransactions').click(async function deleteTransactions(a) {
            ...
            if(retVal === true){
                ...
                if(dataArray.length == 0){
                    $(this).text('Delete Transaction(s)').attr({'disabled':false});
                    showSuccessToaster('Please select at least one Transaction(s) to continue', 'warning');
                    return;
                }
    
                let postData = await postRequest(baseUrl+'api/deleteTransaction', {dataArray:dataArray.join('|')});
